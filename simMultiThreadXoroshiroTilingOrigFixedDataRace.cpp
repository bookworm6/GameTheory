...
// Compile: g++ -O3 -std=c++17 simMultiThreadXoroshiroTilingOrigFixedDataRace.cpp -o simMultiThreadXoroshiroTilingOrigFixedDataRace -pthread
...
int playMove(int theirPrev, int myPrev, double seed, int roundNum, const array<double,4>& rule){ //I wonder what the branch mispredictions are like
    // if (roundNum == 0) {
    //     prevMove = startMove;
    //     return startMove;
    // } Omit in favor of doing this check when running the individual game, rather than at every call to playMove
    int key = (myPrev << 1) | theirPrev;
            ...
            for (int idy = startY; idy < endY; ++idy) {
                for (int idx = startX; idx < endX; ++idx) { //endY is exclusive
                    ...
                    for (int n = 0; n < iters; ++n) {
                        int a1move = playMove(a2prev, a1prev,local_uniform01(), n,a1->rule);
                        int a2move = playMove(a1prev, a2prev,local_uniform01(), n ,a2->rule);
