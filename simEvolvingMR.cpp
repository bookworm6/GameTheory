...
struct Memory1 {
    ...
    void setMR(double _mr) {
        mutationRate = _mr;
    }
};

struct BLANK : public Memory1 {
    BLANK(Move start = COOP, double _mr=0.0) : Memory1(start, _mr) {
        name = "BLANK";
        rule[0] = 0.0;
        rule[1] = 0.0;
        rule[2] = 0.0;
...
AgentGrid blankGrid(int N, pair<int,int> res, unsigned seed = 0, double masterMutationRate = 0.001) {
    ...
    for (int i=0;i<N;++i) for (int j=0;j<N;++j) {
        ...
        ag->setRule(ruleVals);
        double v = paramMaps[4][i][j];
        double mr = v*v;
        // clamp [0,1]
        if (mr < 0.0) mr = 0.0;
        if (mr > 1.0) mr = 1.0;
        ag->setMR(mr*masterMutationRate);
        grid[i][j] = ag;
    }
    return grid;
}
...
TorusResult torusTournament(AgentGrid agentGrid, int iters, int rounds, int snaps, float evolutionRate, 
    ...
    for (int round=0; round<rounds; ++round) {
        ...
        if (round == 1 || ((round % snapEvery) == 0 && (round / snapEvery) > 0)) {
            ...
            size_t ruleLen = 5;// 4 rules + 1 mutationRatwe
