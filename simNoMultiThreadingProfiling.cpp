...
// Compile: g++ -O3 -std=c++17 simNoMultiThreadingProfiling.cpp -o simOneThreadProfile 
// Run: ./sim
//
// Outputs CSV files:
...
void multiThreadedMatchups(AgentGrid& agentGrid, int iters, int rounds, int snaps, int xLen, int yLen, vector<vector<pair<int,int>>>&matchups,vector<vector<int>>& playedTracker,vector<vector<double>>& scoreTracker ){
...
void syncingThreads(int xLen, int yLen,vector<vector<int>>& playedTracker, vector<vector<double>>& scoreTracker, vector<vector<double>>& totalScore){
...
TorusResult torusTournament(AgentGrid agentGrid, int iters, int rounds, int snaps, float evolutionRate, //could agentGrid be passed by reference?
    ...
    for (int round=0; round<rounds; ++round) {
        ...
        multiThreadedMatchups(agentGrid, iters, rounds, snaps, xLen, yLen,matchups,playedTracker,scoreTracker );

        syncingThreads(xLen, yLen, playedTracker, scoreTracker, totalScore);
