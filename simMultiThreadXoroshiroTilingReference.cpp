...
// Compile: g++ -O3 -std=c++17 simMultiThreadXoroshiroTilingReference.cpp -o simMultiThreadXoroshiroTilingReference -pthread
...
TorusResult torusTournament(AgentGrid agentGrid, int iters, int rounds, int snaps, float evolutionRate, //could agentGrid be passed by reference?
    ...
    for (int round=0; round<rounds; ++round) {
        ...
        auto worker = [](int t_id, int rngSeed, int startY, int startX, int endY, int endX, vector<vector<double>>& scoreTracker_local,vector<vector<int>>& playedTracker_local, const AgentGrid& agentGridRef, const vector<vector<pair<int,int>>>& matchupsRef,int iters ) {
            ...
            for (int idy = startY; idy < endY; ++idy) {
                for (int idx = startX; idx < endX; ++idx) { //endY is exclusive
                    ...
                    pair<int,int> match = matchupsRef[idy][idx];
                    auto a1 = agentGridRef[idy][idx];
                    auto a2 = agentGridRef[match.second][match.first];
        ...
        for (int startY=0;startY<=lastStart;startY+=tileSize){
            for (int startX = 0; startX<=lastStart;startX+=tileSize) {
                ...
                threads.emplace_back(worker,threadid,global_rng(),startY,startX,startY+tileSize,startX+tileSize,ref(scoreTracker_threads[threadid]),ref(playedTracker_threads[threadid]),cref(agentGrid),cref(matchups),iters);
                        //auto worker = [&](int t_id, int rngSeed, int startY, int startX, int endY, int endX, vector<vector<double>>& scoreTracker_local,vector<vector<int>>& playedTracker_local, const AgentGrid& agentGridRef, const vector<vector<pair<int,int>>>& matchupsRef ) {
