...
// Compile: g++ -O3 -std=c++17 simMultiThreadXoroshiroReuseRandomCompiletimeMaxReciprocal.cpp -o simMultiThreadXoroshiroReuseRandomMaxReciprocal -pthread
// Run: ./sim
//
// Outputs CSV files:
...
TorusResult torusTournament(AgentGrid agentGrid, int iters, int rounds, int snaps, float evolutionRate, //could agentGrid be passed by reference?
    ...
    for (int round=0; round<rounds; ++round) {
        ...
        auto worker = [&](int t_id, int startRow, int endRow) {
            ...
            constexpr float maxReciprocal = 1/((float)(((uint64_t)(-1))>>48)); //this will be the max that an integer with 16 bits could be. 
