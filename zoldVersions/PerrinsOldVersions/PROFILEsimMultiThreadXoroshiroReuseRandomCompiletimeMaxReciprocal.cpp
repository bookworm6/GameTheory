...
std::tuple<double,double,double,double> local_uniform01(XoshiroCpp::Xoroshiro128Plus& rng){
    constexpr firstmask = (((uint64_t)(-1))>>48);
    constexpr secondmask = (((uint64_t)(-1))>>48)<<16;
    constexpr thirdmask = (((uint64_t)(-1))>>48)<<32;
    constexpr fourthmask = (((uint64_t)(-1))>>48)<<48
    constexpr double maxReciprocal = 1/((double)(((uint64_t)(-1))>>48)); //this will be the reciprocal of the max that an integer with 16 bits could be.
    
    uint64_t currentRandomNumber = rng();

    return std::make_tuple<double,double,double,double>(((firstmask & currentRandomNumber)/maxReciprical),(((secondmask & currentRandomNumber)>>16)/maxReciprical),(((thirdmask & currentRandomNumber)>>32)/maxReciprical),(((fourthmask & currentRandomNumber)>>48)/maxReciprical))
}
