...
// Compile: g++ -O3 -g -std=c++17 simMultiThreadXoroshiroProfiling.cpp -o PROFILEMultiThreadXoroshiro -pthread
// Run: ./sim
//
// Outputs CSV files:
...
// void multithread(vector<thread>& threads){
//         for (auto &th : threads) if (th.joinable()) th.join();
//         threads.clear();
// }

void multithreadAndOverhead(vector<vector<pair<int,int>>>& matchups, vector<vector<int>>& playedTracker, vector<vector<double>>& scoreTracker,int xLen,int yLen,AgentGrid& agentGrid, int iters){
...
TorusResult torusTournament(AgentGrid agentGrid, int iters, int rounds, int snaps, float evolutionRate, //could agentGrid be passed by reference?
    ...
    for (int round=0; round<rounds; ++round) {
        ...
        multithreadAndOverhead(matchups,playedTracker,scoreTracker,xLen,yLen,agentGrid,iters);
